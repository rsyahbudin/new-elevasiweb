    ...
    return (
        <>
            <section className="px-5 pb-0 pt-36 md:px-10 md:pt-[170px]">
                ...
                <h1 className="mt-8 text-[clamp(52px,9.5vw,168px)] font-semibold uppercase leading-[0.98] tracking-[-0.035em]" data-reveal="100">
                    {hero.headlineLine1}
                    <br />
            ...
            <section className="px-5 md:px-10">
                <div data-reveal="0">
                    <Placeholder caption={hero.coverCaption} parallax={0.12} className="h-[58vh] md:h-[72vh]" />
                </div>
            </section>
        </>
    );
            ...
            <div className="grid grid-cols-1 items-start gap-7 md:grid-cols-2">
                {featured.map((project, i) => (
                    <Link
                        ...
                        className={`block ${i % 2 === 1 ? 'md:mt-20' : ''}`}
                        data-reveal={(i % 2) * 100}
                    >
                        <Placeholder
                        ...
                        <div className="flex items-baseline justify-between px-0.5 pb-0 pt-4">
                            <span className="text-lg font-semibold tracking-[-0.01em] md:text-xl">{project.title}</span>
        ...
        <section className="px-5 py-20 md:px-10 md:py-[110px]">
            ...
            <div className="flex flex-col">
                {services.map((service, i) => (
                    <div className="grid cursor-pointer grid-cols-[40px_1fr] gap-4 border-t border-[rgba(27,28,26,0.12)] px-2 py-6 hover:bg-[rgb(236,236,232)] md:grid-cols-[80px_1fr_1fr_40px] md:items-center md:gap-6 md:py-[34px]" key={service.number} data-reveal={i * 60}>
                        <span className="font-mono text-[13px] text-[rgb(31,122,70)]">{service.number}</span>
                        <span className="text-[clamp(24px,3vw,44px)] font-semibold uppercase tracking-[-0.02em]">{service.name}</span>
                        <span className="col-start-2 max-w-[420px] text-sm leading-[1.55] text-[rgba(27,28,26,0.55)] md:col-auto">{service.description}</span>
                        <span className="hidden text-xl text-[rgba(27,28,26,0.4)] md:inline">↗</span>
                    </div>
                ))}
            </div>
        </section>
...
function Testimonial({ testimonial, t }) {
    ...
    return (
        <section className="px-5 pb-24 pt-10 text-center md:px-10 md:pb-[130px]">
            ...
            <blockquote className="mx-auto max-w-[980px] text-[clamp(28px,3.6vw,52px)] leading-[1.25] text-[rgb(27,28,26)]" data-reveal="100">
